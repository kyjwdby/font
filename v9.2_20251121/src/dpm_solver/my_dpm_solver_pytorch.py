    ...
    def model_fn(x, t_continuous):
        ...
        if guidance_type == "uncond":
            ...
            if guidance_scale == 1. or unconditional_condition is None:
                ...
                # 只有当条件列表长度足够时才添加骨架图像条件
                if len(unconditional_condition) > 2 and len(condition) > 2:
                    c_in.append(torch.cat([unconditional_condition[2], condition[2]], dim=0))
                
                ###########################
                # c_in.append(torch.cat([unconditional_condition[3], condition[3]], dim=0))
                # 确保条件列表有第4个元素
                if len(condition) > 3:
                    c_in.append(condition[3])  # for style_feature_cond, no uncond (already unconded from data)
                ###########################
                # 只取模型返回值的第一个元素（噪声预测）进行chunk操作
                model_output = noise_pred_fn(x_in, t_in, cond=c_in)
                if isinstance(model_output, tuple):
                    noise_pred = model_output[0]
                else:
                    noise_pred = model_output
                noise_uncond, noise = noise_pred.chunk(2)
