...
from .my_dpm_solver_pytorch import (NoiseScheduleVP, 
                                model_wrapper, 
                                DPM_Solver)
...
class FontDiffuserDPMPipeline():
    ...
    def generate(
        ...
        # 只有当skeleton_images不为None时才添加到条件列表中
        if skeleton_images is not None:
            cond.append(skeleton_images)
        
        cond.append(style_feature_cond)   #####
        ...
        # 只有当skeleton_images不为None时才创建uncond_skeleton_images
        if skeleton_images is not None:
            uncond_skeleton_images = torch.ones_like(skeleton_images).to(self.model.device)
            uncond.append(uncond_skeleton_images)
            
        ##### for style_feature_cond, no uncond (already unconded from data)
        # uncond_style_feature = torch.ones_like(style_feature_cond).to(self.model.device)
        # uncond.append(uncond_style_feature)
        #####
