...
class VectorQuantizer2(nn.Module):
  ...
  def forward(self, z, temp=None, rescale_logits=False, return_logits=False):
    ...
    # s0 = min_encoding_indices.shape[0]
    # s = int(s0 ** 0.5)
    # s_half = s//2
    # # min_encoding_indices.view(s, s)[:s//2, :s//2] = 15
    # min_encoding_indices.view(s, s)[:, s_half:s_half+1] = 15
    # min_encoding_indices = min_encoding_indices.view(s0)
...
class EMAVectorQuantizer(nn.Module):
  ...
  def __init__(self, n_embed, embedding_dim, beta, decay=0.99, eps=1e-5, remap=None, unknown_index="random"):
    ...
    self.embedding_dim = embedding_dim
    self.beta = beta
    self.embedding = EmbeddingEMA(self.n_embed, self.embedding_dim, decay, eps)
  ...
  def forward(self, z):
    ...
    d = z_flattened.pow(2).sum(dim=1, keepdim=True) + \
      self.embedding.weight.pow(2).sum(dim=1) - 2 * torch.einsum('bd,nd->bn', z_flattened, self.embedding.weight)  # 'n d -> d n'
