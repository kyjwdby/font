...
class DropKeyMask(nn.Module):
  ...
  def _check_row(self, mask: torch.Tensor):
    ...
    droped_row = mask.sum(-1, dtype=torch.bool).flatten()  # RuntimeError: CUDA error: device-side assert triggered
    droped_row = droped_row.logical_not_().argwhere().flatten()
    # droped_row = mask.sum(-1).flatten()  # ！！！！！！！！！！！！！！！！！！！
    # droped_row = (droped_row == 0).nonzero(as_tuple=False).flatten()  # ！！！！！！！！！！！！！！！！！！！
