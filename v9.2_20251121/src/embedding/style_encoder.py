...
class StyleEncoder(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        self.init_weights()
        #################################################
        self.style_reducer = torch.nn.Linear(self.ch*16, 256)  # 1024->256
    ...
    def forward(self,x):        
        ...
        style_emd = h        
        ###########################################
        batch_size, channel, height, width = style_emd.shape
        style_reduce = style_emd.permute(0, 2, 3, 1).reshape(batch_size, height*width, channel)
        style_reduce = self.style_reducer(style_reduce)
        style_reduce = style_reduce.mean(dim=1)  # [B, 256]
        ###########################################
        h = F.adaptive_avg_pool2d(h,(1,1))
        h = h.view(h.size(0),-1)
        
        # return style_emd,h,residual_features
        return style_emd,style_reduce,h,residual_features
