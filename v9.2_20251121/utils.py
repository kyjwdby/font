...
def save_image_with_content_style(save_dir, image, content_image_pil, content_image_path, style_image_path, target_image_path, resolution):
    
    # new_image = Image.new('RGB', (resolution*3, resolution))
    new_image = Image.new('RGB', (resolution*4, resolution))
    ...
    target_image = Image.open(target_image_path).convert("RGB").resize((resolution, resolution), Image.BILINEAR)

    new_image.paste(content_image, (0, 0))
    new_image.paste(style_image, (resolution, 0))
    new_image.paste(image, (resolution*2, 0))
    new_image.paste(target_image, (resolution*3, 0))
...
########### self add
def save_image_with_content_allstyle(save_dir, image, content_image_pil, content_image_path, style_image_path, target_image_path, resolution):
    
    # new_image = Image.new('RGB', (resolution*3, resolution))
    new_image = Image.new('RGB', (3*resolution*4 + int(2*resolution*0.5), resolution*12))  # 3 colomns, 12 rows
    if content_image_pil is not None:
        content_image = content_image_pil
    else:
        content_image = Image.open(content_image_path).convert("RGB").resize((resolution, resolution), Image.BILINEAR)
    ...
    for i in range(len(image)):
        style_image = Image.open(style_image_path[i]).convert("RGB").resize((resolution, resolution), Image.BILINEAR)
        target_image = Image.open(target_image_path[i]).convert("RGB").resize((resolution, resolution), Image.BILINEAR)

        start_x = resolution*4*(i%3)+int(resolution*0.5)*(i%3)
        start_y = resolution*(i//3)
        new_image.paste(content_image, (start_x, start_y))
        new_image.paste(style_image, (start_x+resolution, start_y))
        new_image.paste(image[i], (start_x+resolution*2, start_y))
        new_image.paste(target_image, (start_x+resolution*3, start_y))

    save_path = f"{save_dir}/out_all_with_cs.jpg"
    new_image.save(save_path)


def save_image_with_content_allstyle_v1(save_dir, image, content_image_pil, content_image_path, style_image_path, target_image_path, resolution):
    
    # new_image = Image.new('RGB', (resolution*3, resolution))
    colomn, row = 12, 24
    new_image = Image.new('RGB', (colomn*resolution*4 + int((colomn-1)*resolution*0.5), resolution*row))  # 3 colomns, 12 rows
    if content_image_pil is not None:
        content_image = content_image_pil
    else:
        content_image = Image.open(content_image_path).convert("RGB").resize((resolution, resolution), Image.BILINEAR)
    ...
    for i in range(len(image)):
        ...
        start_x = resolution*4*(i%colomn)+int(resolution*0.5)*(i%colomn)
        start_y = resolution*(i//colomn)
        new_image.paste(content_image, (start_x, start_y))
        new_image.paste(style_image, (start_x+resolution, start_y))
        new_image.paste(image[i], (start_x+resolution*2, start_y))
        new_image.paste(target_image, (start_x+resolution*3, start_y))

    save_path = f"{save_dir}/out_all_with_cs_v1.jpg"
    new_image.save(save_path)
